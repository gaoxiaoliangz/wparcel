...
/**
 * @param {{features: { polyfill, react, sass, node, css, postcss, babel, typescript, nodeExternals }}} config 
...
function generateConfig(config, webpackConfig) {
  ...
  if (features.nodeExternals) {
    const nodeExternalsConfig = features.nodeExternals.config || featureDefinitions.config

...
const { mergeAnything, resolveProject, endsWith } = require('../utils')
const { mergeBabelOptions } = require('./merge-babel-options')
...
class Feature {
  ...
  constructor({ userConfig, features, defaultConfig, dependency, context }) {
    ...
    this.defaultConfig = _.omitBy(defaultConfig, (v, k) => endsWith(k, '_doc'))
...
class Babel extends Feature {
  constructor(arg) {
    const enableCssModules = _.get(arg, 'features.sass.scoped') || _.get(arg, 'features.css.scoped')
    super(Object.assign({}, arg, {
      defaultConfig: Babel.defaultConfig,
      dependency: babelDep.concat(enableCssModules
    ...
    this._enableCssModules = enableCssModules
  }
  ...
  eval() {
    ...
    const options = typeof this.config.options === 'function'
      ...
      : mergeBabelOptions(mergeBabelOptions(this.defaultConfig.options, {
        plugins: [
          /**
           * https://github.com/gajus/babel-plugin-react-css-modules#configurate-syntax-loaders
           */
          ...this._enableCssModules
            ? [['react-css-modules', {
...
Babel.defaultConfig = {
  ...
  options_doc: 'whether to use babelrc config file or not'
}
...
class Define extends Feature {
  ...
  eval() {
    const config = Object.assign({}, this.config, {
      'process.env.NODE_ENV': this.context.production ? 'production' : 'development'
    })

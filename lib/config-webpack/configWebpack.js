...
const { isDepInstalled, printErrorAndExit, printWarning, mergeAnything, capFirstLetter, resolveProject, copyFileWithExistenceCheck } = require('../utils')
...
const validateUserConfig = (features, userConfig) => {
  ...
  if (features.find(f => f.key === 'typescript')) {
    try {
      ...
      printErrorAndExit('When enabling typescript, tsconfig.json is requierd!\nYou can use `jellyweb init --ts` to generate one')
    }
  }
}

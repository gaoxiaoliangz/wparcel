 ...
 * @param {Function[]} features Function userConfig => { feature, userConfig }
...
function configWebpack(features, webpackConfig) {
  ...
  const features0 = _.flatten(features)
  const features1 = features0.reduce((obj, feature) => {
    return Object.assign({}, obj, {
      [feature.feature]: feature.userConfig || {}
    })
  }, {})
  // feature instances, eg: { name: 'Css': instance: {...} }
  const features2 = features0
    .map(feature => {
  ...
  const targets = _.flatten(features2.map(({ instance }) => instance.eval())).filter(Boolean)
